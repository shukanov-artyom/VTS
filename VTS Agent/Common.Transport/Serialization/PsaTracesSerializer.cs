...
using Common.Transport.Resources;
using VTS.Shared;
using VTS.Shared.DomainObjects;

namespace Common.Transport.Serialization
{
    internal class PsaTracesSerializer
    {
        ...
        private const string AdditionalSourceInfoName = "AdditionalSourceInfo";
        private const string OriginalTypeId = "OriginalTypeId";
        private const string OriginalNameId = "OriginalName";
        ...
        private PsaTrace ReadTraceElement(XElement traceElement)
        {
            ...
            // on resources
            XElement id = traceElement.Element(mgr.GetString(IdName));
            XElement psaDatasetId = traceElement.
                Element(mgr.GetString(PsaDatasetIdName));
        ...
        private void FillTrace(XElement traceElement, PsaTrace sourceTrace)
        {
            ...
            application.Value = sourceTrace.Application ?? String.Empty;
            ...
            PsaParametersSet sourceParamSet)
        {
            ...
            foreach (PsaParameterData data in sourceParamSet.Parameters)
            {
                ...
                XElement originalTypeIdElement = new XElement(OriginalTypeId);
                originalTypeIdElement.Value = data.OriginalTypeId;
                XElement additionalSourceInfoElement = new XElement(AdditionalSourceInfoName);
                additionalSourceInfoElement.Value = data.AdditionalSourceInfo;
                parameterElement.Add(originalTypeIdElement);
                
                XElement originalNameElement = new XElement(OriginalNameId);
                originalNameElement.Value = data.OriginalName;
                parameterElement.Add(originalNameElement);
